...
void Sidebar::show_SEMM_buttons()
{
    ...
    const bool show_cfs = is_cfs_supported_printer() && (!get_cfs_socket_host().empty() || p->cfs_auto_sync_enabled);
    if (p->m_bpButton_cfs_filament)
        p->m_bpButton_cfs_filament->Show(show_cfs);
    if (p->m_link_cfs_config)
        p->m_link_cfs_config->Show(show_cfs);
